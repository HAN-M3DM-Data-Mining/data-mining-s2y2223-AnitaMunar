...
I see that the dataset has 20800 observations and 5 variables. It consists of short news, with authors and most importantly: a label of 0 or 1. This is an indication that the fake news and real news have been already identified manually. My goal is to predict based on given specific words - which news are fake.
...
After checking the data, I see the raw text data of the observed news, id numbers, authors, and most importantly the data called "label", with values of either 0 or 1. The "title", "author" and "text" are Character data.
...
Now to look at the data using a wordcloud, the above code is used. It gives the error message as seen in class, being:
    ...
    2: In tm_map.SimpleCorpus(corpus, function(x) tm::removeWords(x, tm::stopwords())) :
      transformation drops documents
Regardless, with the world cloud working we have a better view of the data.
...
The error comes up as seen in class:
    ...
    transformation drops documents
This is not a great issue. The command still works. It removes stopwords such as "and" and "but" because those words likely exist in both fake and real news.
In reflection, the wordcloud is more likely to be useful if the command is ran after the cleanCorpus command, since the stopwords made it difficult for the computer to come up with the wordcloud.
